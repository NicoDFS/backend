...
import {
  MINIMUM_LIQUIDITY_THRESHOLD_KLC,
...
export function getKlcPriceInUSD(): BigDecimal {
  ...
  for (let i = 0; i < STABLE_TOKEN_PAIRS.length; i++) {
    ...
    if (stableTokenPair) {
      ...
      if (stableTokenIsToken0[i]) {
        ...
        totalLiquidityKLC = totalLiquidityKLC.plus(stableTokenPair.reserve1)
      } else {
        stableTokenReserves[i] = stableTokenPair.reserve0
        stableTokenPrices[i] = stableTokenPair.token1Price
        totalLiquidityKLC = totalLiquidityKLC.plus(stableTokenPair.reserve0)
      }
    }
  ...
  for (let i = 0; i < STABLE_TOKEN_PAIRS.length; i++) {
    if (stableTokenPairs[i] !== null) {
      tokenPrice = tokenPrice.plus(stableTokenPrices[i].times(safeDiv(stableTokenReserves[i], totalLiquidityKLC)))
    }
  }
  return tokenPrice
}
...
export function findKlcPerToken(token: Token): BigDecimal {
  ...
  if (STABLECOINS.includes(token.id)) {
    ...
    return safeDiv(ONE_BD, bundle.klcPrice)
  }
