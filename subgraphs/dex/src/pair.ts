...
export function handleSync(event: SyncEvent): void {
  ...
  // Update factory total liquidity by tracking the change in this pair's liquidity
  let oldReserveUSD = ZERO_BD;
  let oldReserveKLC = ZERO_BD;

  // If we have previous values stored, use them
  if (pair.reserveUSD && pair.reserveKLC) {
    oldReserveUSD = pair.reserveUSD;
    oldReserveKLC = pair.reserveKLC;
  }

  // Calculate the delta (change) in liquidity
  let deltaUSD = pair.reserveUSD.minus(oldReserveUSD);
  let deltaKLC = pair.reserveKLC.minus(oldReserveKLC);

  // Update factory liquidity by adding the delta
  factory.totalLiquidityUSD = factory.totalLiquidityUSD.plus(deltaUSD);
  factory.totalLiquidityKLC = factory.totalLiquidityKLC.plus(deltaKLC);

  // Ensure we never go below zero
  if (factory.totalLiquidityUSD.lt(ZERO_BD)) {
    factory.totalLiquidityUSD = ZERO_BD;
  }
  if (factory.totalLiquidityKLC.lt(ZERO_BD)) {
