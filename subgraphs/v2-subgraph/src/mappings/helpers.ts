...
// dont count in tracked volume
export let UNTRACKED_PAIRS: string[] = ['0x37eA64bB4D58b6513C80bEFA5Dc777080AD62EB9']
...
export function convertKlcToDecimal(klc: BigInt): BigDecimal {
  return klc.toBigDecimal().div(exponentToBigDecimal(18))
}
...
export function fetchTokenSymbol(tokenAddress: Address): string {
  ...
  if (symbolResult.reverted) {
    ...
    if (!symbolResultBytes.reverted) {
      ...
      if (!isNullKlcValue(symbolResultBytes.value.toHexString())) {
...
export function fetchTokenName(tokenAddress: Address): string {
  ...
  if (nameResult.reverted) {
    ...
    if (!nameResultBytes.reverted) {
      ...
      if (!isNullKlcValue(nameResultBytes.value.toHexString())) {
