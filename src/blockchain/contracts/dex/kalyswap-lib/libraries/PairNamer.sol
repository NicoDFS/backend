...
// produces names for pairs of tokens using Kalyswap's naming scheme
library PairNamer {
    string private constant TOKEN_SYMBOL_PREFIX = 'KSWAP:';
    ...
    // produces a pair symbol in the format of `KSWAP:${symbol0}:${symbol1}${suffix}`
    function pairSymbol(
        address token0,
        address token1,
        string memory suffix
